...
async function testOpenCageAPI() {
  try {
    ...
    const forwardUrl = `https://api.opencagedata.com/geocode/v1/json?key=${API_KEY}&q=London&limit=1`;
    const forwardResponse = await fetch(forwardUrl);
    const forwardData = await forwardResponse.json();
    
    if (forwardData.results && forwardData.results.length > 0) {
      console.log("✅ Forward geocoding test passed");
      console.log(`   London coordinates: ${forwardData.results[0].geometry.lat}, ${forwardData.results[0].geometry.lng}`);
    } else {
      console.log("❌ Forward geocoding test failed");
    }
    
    // Test reverse geocoding
    const reverseUrl = `https://api.opencagedata.com/geocode/v1/json?key=${API_KEY}&q=51.5074,-0.1278&limit=1`;
